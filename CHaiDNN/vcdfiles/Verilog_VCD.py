            ...
            if line[0] in ('b', 'B', 'r', 'R'):
                ...
                if not "$end" in line:
                    while fh:
                        ...
                        if "$end" in line:
                            break
                # print('Timescale statement: ', statement)
                # print('Opt timescale: ', opt_timescale)
                
                mult, mode = calc_mult(statement, opt_timescale)
...
def calc_mult (statement, opt_timescale=''):
    ...
    if ts_match:
        ...
        # print('tscale = ' + tscale + ', mult = ' + str(mult) + ', unit = ' + units)
    
    else:
        VCDParseError("Error: Unsupported timescale found in VCD "\
                "file: "+tscale+".  Refer to the Verilog LRM.")
    
    # change the dict here so that we will return integers when possible
    mults = {
        'fs' : -15,
        'ps' : -12,
        'ns' : -9,
    ...
    if units in mults:
        ...
        VCDParseError("Error: Unsupported timescale units found in VCD " \
                      "file: " + units + ". Supported values are: fs, ps, ns, us, ms, s. ")
    ...
    if new_units in mults:
        ...
        VCDParseError("Error: Illegal user-supplied " \
                      "timescale: " + new_units + ". Legal values are: fs, ps, ns, us, ms, s. ")

    # some other effort to make it return integers
    # create a "mode" variable to show whether we are switching to a larger timescale or not
    if new_scale >= scale:
      mode = 'large'
      multiplier = 10 ** (new_scale - scale)
      if multiplier % mult == 0:
        result = multiplier // mult
      else:
        result = multiplier / mult
    else:
      mode = 'small'
      multiplier = 10 ** (scale - new_scale)
      result = multiplier * mult
