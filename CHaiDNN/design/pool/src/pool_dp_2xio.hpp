...
#pragma HLS loop_flatten off
		...
		//(wtReg, wts+i, MEM_BYTES_WTS);
		for (int j = 0; j < MEM_BYTES_WTS; j++) {
			*(wtReg+j) = *(wts+i+j);
		}
		PACK1:
...
#pragma HLS loop_flatten off
		...
		//(iReg1, in1+i, MEM_BYTES);
		//(iReg2, in2+i, MEM_BYTES);
...
void poolKernel(
		GMEM_MAXPOOLTYPE *inc1, GMEM_MAXPOOLTYPE *in2, GMEM_MAXPOOLTYPE *out1, GMEM_MAXPOOLTYPE *out2, GMEM_POOLTYPE_WTS * wts,
...
#pragma HLS loop_tripcount min=1 max=256
			...
			PlanesPingPong(
					inc1, in2, out1, out2, wts, offset_in, offset_out,
