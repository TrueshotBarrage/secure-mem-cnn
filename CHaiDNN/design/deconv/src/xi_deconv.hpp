...
#pragma HLS PIPELINE
				...
				//for (int i = 0; i < SIZEOF(INTYPE_FLOAT); i++) {
				//	*(&tmp+i) = *(in+ddrIndex+i); 
				//}
				//tmp = 256;
...
//				dim3 = (n/DECONV_Tn)*(KMAX/2) * (KMAX/2) + dim3i*DS + dim3j;
				dim3 = dim3i*DS + dim3j;
				//(&tmp, in+ddrIndex, SIZEOF(WTYPE));
				for (int i = 0; i < SIZEOF(WTYPE); i++) {
					*(&tmp+i) = *(in+ddrIndex+i);
				}
...
#pragma HLS PIPELINE
		...
		//(deconvIDout + i, &tmp, 8);//SIZEOF(OUTIDTYPE));
		//deconvIDout[i] = tmp;
	}
}
